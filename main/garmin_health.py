...
class GarminHealth(object):
    ...
    def __validate_conditions(self):
        ...
        if (self._consumer_key is None or self._consumer_secret is None) and self._import_credentials is False:
            raise ValueError(
                """
    ...
    def fetch_authorization_url(self, oauth_callback):
        ...
        _LOGGER.debug("Authorizing OAuth1 (2/3) token to %s",
                      AUTHORIZE_TOKEN_URL)
        authorization_url = self.oauth.authorization_url(AUTHORIZE_TOKEN_URL, oauth_callback=oauth_callback)

        return authorization_url

    def complete_garmin(self, redirect_response, resource_owner_secret):

        throwaway_oauth = OAuth1Session(client_key=self._consumer_key, client_secret=self._consumer_secret)

        oauth_response = throwaway_oauth.parse_authorization_response(redirect_response)
        ...
        _LOGGER.debug("Fetching OAuth1 (3/3) token to %s", ACCESS_TOKEN_URL)
        self.oauth.fetch_access_token(ACCESS_TOKEN_URL)

        _LOGGER.debug(f"Got oauth response {oauth_response}")
